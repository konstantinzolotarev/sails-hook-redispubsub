...
module.exports = function PrepareConnection(app, cb) {
  ...
  // Create connections
  app.redispubsub.subClient = config.subClient || redis.createClient(config.connection.options);
  app.redispubsub.pubClient = config.pubClient || redis.createClient(config.connection.options);
  ...
  app.redispubsub.subClient.on('message', function(channel, message) {
    ...
    if (_.isObject(message) && _.isString(message.event)) {
      app.redispubsub.emit(message.event, _.omit(message, 'event'));
    }
    // Check and call method
